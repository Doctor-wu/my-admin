...
let BannerForm: any = (props: any, ref: any) => {
    ...
    useEffect(() => {
        BannerApi.getBannerCosAllImages().then(res => {
            setImages(res.data);
            setLoading(false);
        })
    }, [])
    ...
    const update = () => {
        return new Promise(((resolve, reject) => {
            ...
            formRef.current.validateFields().then(res => {
                console.log(res)
                // BannerApi.uploadBanner(Object.assign(res, {
                //     id: props.fields.find((field:any)=>field.name === "id")["value"]-1
                // }))
                // @ts-ignore
                BannerApi.uploadBanner(Object.assign(res, {
                        ...
                        : <Radio.Group>
                            {images.map(image => {
                                return (
                                    <>
                                        <Radio value={image}>
                                            <Image style={{margin: "5px"}} width={175} src={image}/>
                                        </Radio><br/>
                                    </>
                                )
                            })}
...
BannerForm = forwardRef(BannerForm);
